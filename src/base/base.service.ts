...
export class BaseService<T> {
  ...
  constructor(@InjectModel(BaseService.name) protected model: Model<any>) {
  ...
  async find(
    ...
    return <T[]>await query.exec().then((docs) => {
      let result = docs.map((doc) => {
        if (doc.balance) {
          doc.balance = parseFloat(doc.balance);
        }
        return doc;
      });
      return result;
  ...
  async create(dto: unknown = {}): Promise<T | BaseEntity> {
    ...
    const entity = await new this.model(dto);
    return entity.save();
  }
  ...
  async updateByCondition(
    ...
    const current_information = await this.model
      .findOne({ ...condition })
      .exec();
